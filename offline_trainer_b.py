...
def run_offline_training_b(stock_list_csv_path, market_type, config):
    ...
    logger.info(f"  優化方法: {'NSGA-II 多目標優化' if config.ENABLE_NSGA2 else '傳統單目標GA'}")
    ...
    if config.ENABLE_NSGA2 and not NSGA2_AVAILABLE:
        logger.warning("⚠  NSGA-II 已啟用但 pymoo 未安裝，自動切換為傳統 GA")
    ...
    for i, ticker_raw in enumerate(tickers_to_train):
        ...
        if i > 0 and i % config.STOCKS_PER_BATCH == 0:
            ...
            logger.info(f"\n  已處理 {i} 支股票，暫停 {config.PAUSE_DURATION_MINUTES} 分鐘...")
        ...
        try:
            ...
            if not prices or len(prices) < 100:
                logger.warning(f"⚠  {ticker_raw} 數據不足或載入失敗 (已嘗試 .TW/.TWO)，跳過處理")
                failed_trainings += 1
                continue
                
            logger.info(f" 成功載入 {ticker} 的 {len(prices)} 個交易日數據")

            logger.info(f"  正在預計算技術指標...")
            ...
            if not indicator_ready:
                logger.warning(f"  {ticker} 技術指標預計算失敗，跳過處理")
                failed_trainings += 1
                continue
            ...
            if not strategy_pool:
                logger.warning(f"  未找到有效策略，跳過 {ticker}")
                failed_trainings += 1
                continue
            ...
            if not top_champions:
                logger.warning(f"  無法確定冠軍策略，跳過 {ticker}")
                failed_trainings += 1
                continue
...
def analyze_training_scope_b(config):
    ...
    for market_name, file_path in config.STOCK_LIST_FILES.items():
        if not config.MARKETS_TO_TRAIN.get(market_name, False):
            logger.info(f"  跳過 {market_name}: 已停用")
            continue
        ...
        try:
            ...
            if symbol_col:
                ...
                logger.warning(f"⚠  {market_name}: 找不到股票代號欄位 - {file_path}")
    ...
    logger.info(f"     預估計算時間: {estimated_time_hours:.1f} 小時")
    logger.info(f"     預估暫停時間: {pause_time_hours:.1f} 小時")
    ...
    if total_estimated_hours > 48:
        logger.warning(f"  預估時間超過2天，建議考慮:")
...
if __name__ == "__main__":
    ...
    logger.info(f"⚙  優化引擎: {'NSGA-II 多目標優化' if config.ENABLE_NSGA2 else '傳統遺傳算法'}")
    logger.info(f" 基因系統: 10基因RSI主導策略")
    logger.info(f" 新功能: 單次交易最大跌幅/漲幅分析")
